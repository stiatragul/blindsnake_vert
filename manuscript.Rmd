...
The Australian blindsnake radiation comprise 47 described species [@tiatragulAustralia2023] with exceptionally diverse morphology from slender (aspect ratio = ) to robust. Body proportion exhibit strong phylogenetic signal but in some instances spread out across the phylogeny (CITE). 

In this study, we investigate the interspecific variation in the number of pre- and post-cloacal vertebrae to test the hypothesis that changes in developmental patterns underpin the evolution of different body proportions in Australian typhlopid snakes. 
...
We did this by testing whether the total number of vertebrae positively correlate with body size when regressing the maximum number of total vertebrae against maximum SVL from Tiatragul et al. (unpublished). Then, we test whether the total vertebrae number to total body length ratio correlates with body shape, defined as the ratio between midbody diameter divided by SVL, also from Tiatragul et al. (unpublished) data. 
...
We found no relationship between indicating maximum number of total vertebrae and maximum SVL (PGLS, slope = -0.08, y=225, R^2^=0.02, F=0.9, Z=0.40, P=0.361), indicating an absence of pleomerism in Australian blindsnakes. 

Vertebrae ratio is negatively correlated with body width ratio (PGLS, slope = -21.4, y=1.4, R^2^=0.2, F=8.8,, Z=2.3, P=0.007), indicating that species that are more narrow-and-wide usually have greater total vertebrae to total length ratios than species that are wide-and-robust (Fig. \@ref(fig:fig-vert-width-ratios)). 
...
<!-- Figure 2: Pleomerism and ratio correlations -->
```{r fig-vert-width-ratios, fig.cap= 'Relationship between mean vertebrae ratio and mean body width ratio among Australian blindsnakes. Vertebrae ratio is calculated by total vertebrae/total length, and body width ratio is midbody diameter/total body length.', out.width="165mm", out.height= "126mm"}

knitr::include_graphics(paste0(WD, '/assets/figures/vert-width-ratios.jpg'))
```
