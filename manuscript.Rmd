...
We used the Fiji distribution of ImageJ2 [@schindelinFiji2012] to digitally visualise X-ray images and counted the number of vertebrae using Cell Counter module. Precloacal and postcloacal vertebrae were distinguished based on vertebral morphology. 

Additionally, we quantified the vertebral profile for two representative blindsnake species (*A. grypus* and *A. ligatus*) by measuring the length of each vertebra from the first pre-cloacl vertebra to the last tail vertebra. Following @sherrattMacroevolution2022 we used the 'multipoint tool' in ImageJ2 where landmarks were placed medially along the vertebral column at the anterior limit of the centrum of each vertebra. 
...
Occurrence data were based on locality records of preserved specimens from museum databases and verified human observations from Atlas of the Living Australia (ALA). We modified some records to reflect the latest taxonomic changes based on updated distribution range and ST's examination of specimens. We discarded records with identical latitude and longitude, and records with uncertain taxonomic identities. In total we retained 3896 verified records representing every species of Australian *Anilios* for our analyses (See data availability statement).
